...
function parseTokenSection($, section, meta) {
  ...
  const parseParagraph = text => {
    ...
    if (text.startsWith(timingPrefix)) {
      current.placement_timing = cleanText(text.slice(timingPrefix.length));
      return;
    }
    if (text.startsWith(conditionPrefix)) {
      current.placement_conditions = cleanText(text.slice(conditionPrefix.length));
      return;
    }
    if (text.startsWith(removalPrefix)) {
      current.removal_timing = cleanText(text.slice(removalPrefix.length));
    }
  };
