  ...
  hooks: {
    on_night_wake: (context): PluginResult => {
          ...
          emitted_events: [
            {
              ...
              payload: {
                prompt_key: null,
                note: `librarian_info:${context.player_id}:inactive`
              }
            }
          ],
          queued_prompts: [],
      ...
      const prompt_key = info_mode === 'truthful'
        ? build_librarian_truth_prompt_key(context.state.night_number, context.player_id)
        : build_librarian_misinfo_prompt_key(context.state.night_number, context.player_id);
        ...
        queued_prompts: [
          {
            prompt_key,
            kind: 'choice',
            reason: `plugin:librarian:choose_info:n${context.state.night_number}:${context.player_id}`,
            visibility: 'storyteller',
            options: [],
            selection_mode: 'multi_column',
            multi_columns: [character_ids, player_ids, player_ids],
            storyteller_hint: build_role_pair_note(context.state, context.player_id, 'outsider', 'librarian_info')
          }
        ],
        queued_interrupts: []
      };
    ...
    on_prompt_resolved: (context): PluginResult => {
      if (!is_librarian_truth_prompt_key(context.prompt_key) && !is_librarian_misinfo_prompt_key(context.prompt_key)) {
        return {
          emitted_events: [],
          queued_prompts: [],
          queued_interrupts: []
      ...
      const owner_player_id = parse_night_prompt_owner_player_id(
        context.prompt_key,
        'librarian',
        is_librarian_truth_prompt_key(context.prompt_key) ? 'choose_info_truth' : 'choose_info_misinfo'
      );
      if (!owner_player_id) {
        return {
          emitted_events: [],
      ...
      let note = build_role_pair_note(context.state, owner_player_id, 'outsider', 'librarian_info');

      if (parsed) {
        const mode: 'truthful' | 'misinformation' = is_librarian_truth_prompt_key(context.prompt_key)
          ? 'truthful'
          : 'misinformation';
        const can_use = mode === 'misinformation' || is_valid_truthful_selection(context.state, owner_player_id, parsed);
        if (can_use) {
          note =
            `librarian_info:${owner_player_id}:character=${parsed.character_id};` +
...
function build_librarian_truth_prompt_key(night_number: number, player_id: string): string {
  return build_night_prompt_key('librarian', 'choose_info_truth', night_number, player_id);
}

function build_librarian_misinfo_prompt_key(night_number: number, player_id: string): string {
  return build_night_prompt_key('librarian', 'choose_info_misinfo', night_number, player_id);
}

function is_librarian_truth_prompt_key(prompt_key: string): boolean {
  return is_night_prompt_key(prompt_key, 'librarian', 'choose_info_truth');
}

function is_librarian_misinfo_prompt_key(prompt_key: string): boolean {
  return is_night_prompt_key(prompt_key, 'librarian', 'choose_info_misinfo');
}
...
function list_in_play_character_ids(
  ...
  for (const player of list_players_by_true_character_type(state, 'outsider')) {
